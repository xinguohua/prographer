...
def get_true_snapshot_labels(snapshots):
    """
    为快照生成真实标签
    根据每个快照中节点的 label 属性来确定该快照是否为恶意
    """
    true_labels = []
    for snapshot in snapshots:
        # 检查快照中是否包含恶意节点（label=1）
        malicious_nodes = sum(1 for v in snapshot.vs if v.attributes().get('label', 0) == 1)
        total_nodes = len(snapshot.vs)
        
        # 如果快照中有恶意节点，标记为恶意（1），否则为良性（0）
...
def run_snapshot_level_evaluation(detector_model_path, encoder_model_path, PATH_MAP, MALICIOUS_INTERVALS_PATH,
    ...
    print(f"  - 训练时序列长度: {snapshot_data.get('sequence_length', 'N/A')}")
    
    # 创建虚拟handler用于后续处理

    # 🔥 从加载的数据中提取恶意快照用于测试
    mal_snapshots = all_snapshots[malicious_idx_start:malicious_idx_end]
    
    print(f"🎯 使用恶意快照进行测试:")
    print(f"  - 恶意快照数量: {len(mal_snapshots)}")
    print(f"  - 恶意快照索引范围: {malicious_idx_start} 到 {malicious_idx_end-1}")
    ...
    benign_count = 0
    malicious_count = 0
    for i, (snapshot, label) in enumerate(zip(mal_snapshots, true_labels)):
        mal_nodes = sum(1 for v in snapshot.vs if v.attributes().get('label', 0) == 1)
        total_nodes = len(snapshot.vs)
        if label == 1:
            malicious_count += 1
        else:
            benign_count += 1
        if i < 5:  # 只显示前5个快照的详细信息
            print(f"  快照 {i}: {mal_nodes}/{total_nodes} 恶意节点, 标签={label}")
    
    print(f"标签统计: 恶意快照={malicious_count}, 良性快照={benign_count}")
    ...
    # 🔥 使用恶意快照的嵌入进行异常检测
    malicious_embeddings = snapshot_embeddings[malicious_idx_start:malicious_idx_end]
    pred_labels, diff_vectors = predict_anomalous_snapshots(malicious_embeddings, detector_model_path)
    
    print(f"🔍 异常检测结果:")
    print(f"  - 检测到异常快照数: {len(diff_vectors)}")
    print(f"  - 预测标签数量: {len(pred_labels)}")
    print(f"  - 恶意嵌入数量: {len(malicious_embeddings)}")
    
    # 【参考test_graphs.py】评估恶意快照部分
    eval_true = true_labels
    eval_pred = pred_labels
    
    print(f"✅ 评估恶意快照: {malicious_idx_start} 到 {malicious_idx_end}")
    ...
    print_debug_info(mal_snapshots, eval_true, eval_pred, 0)  # 参考test_graphs.py的调用方式
...
if __name__ == '__main__':
    ...
    run_snapshot_level_evaluation(
        DETECTOR_MODEL_PATH, 
        ENCODER_MODEL_PATH, 
        PATH_MAP, 
        None,  # MALICIOUS_INTERVALS_PATH (参考test_graphs.py)
